...
class DialogueRNNCell(nn.Module):
    ...
    def _select_parties(self, X, indices):
        ...
        for idx, j in zip(indices, X):
            q0_sel.append(j[idx].unsqueeze(0))#unsqueeze是添加一个新的维度，目的是为之后的cat能叠起来，所以torch.cat叠起来其实是有要求的？
    ...
    def forward(self, U, qmask, g_hist, q0, e0):
        """
        U -> batch, D_m
        qmask -> batch, party 用来标识是哪一个发言者说的话
        g_hist -> t-1, batch, D_g
        q0 -> batch, party, D_p
        e0 -> batch, self.D_e
        """
        qm_idx = torch.argmax(qmask, 1)#qmask用来表示这句话属于哪一个发言者，是一个二维张量，但是列数为1，只有一行的值为1，表明为发言者
        q0_sel = self._select_parties(q0, qm_idx) #从发言者状态 q0 中选取当前发言者的状态

        g_ = self.g_cell(torch.cat([U,q0_sel], dim=1),
                torch.zeros(U.size()[0],self.D_g).type(U.type()).to(self.device) if g_hist.size()[0]==0 else
                g_hist[-1])
        g_ = self.dropout(g_)#随机选择一部分元素并将它们设置为零
        if g_hist.size()[0]==0:
            c_ = torch.zeros(U.size()[0],self.D_g).type(U.type()).to(self.device)#c_ -> batch, D_g
            alpha = None
        else:
            c_, alpha = self.attention(g_hist,U)#c_是上下文向量，alpha是注意力分数
        # c_ = torch.zeros(U.size()[0],self.D_g).type(U.type()) if g_hist.size()[0]==0\
        #         else self.attention(g_hist,U)[0] # batch, D_g
        U_c_ = torch.cat([U,c_], dim=1).unsqueeze(1).expand(-1,qmask.size()[1],-1)#U_c batch,party, D_g
